...
export class MediaRatingService {
    ...
    rateMedia$(
        ...
        if (!sessionId && !guestSessionId) {
            return throwError(
                () => new Error('You need a TMDb user session or guest session to rate.'),
            );
        }
        ...
        const request$ =
            mediaType === 'tv'
                ? this.tvSeriesService.tvSeriesAddRating(
                      ...
                      guestSessionId ?? undefined,
                      sessionId ?? undefined,
                      buildRawBody({ value: ratingValue }),
                      'body',
                      false,
                      API_JSON_OPTIONS,
                  )
    ...
    deleteMediaRating$(
        ...
        if (!sessionId && !guestSessionId) {
            return throwError(
                () => new Error('You need a TMDb user session or guest session to remove a rating.'),
            );
        }
